...
import { Plus, X, ChevronDown, ChevronUp, Download, FileText, Loader2, RotateCcw, Lock, User, Camera, Check } from "lucide-react";
...
export default function GradCV() {
  ...
  useEffect(()=>{
    const fit=()=>{ if(!wrapRef.current)return; const w=wrapRef.current.clientWidth; setScale(Math.max(0.3,Math.min(1,(w-32)/(8.5*96)))); };
    ...
    <div className="min-h-screen w-full flex items-center justify-center" style={{ background:"linear-gradient(135deg, #F0EDFF 0%, #E8F4FD 100%)" }}>
      ...
      <div className="w-full max-w-sm mx-auto px-6">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400">Your student resume builder</p>
        </div>
    ...
    <div className="min-h-screen w-full" style={{ background:"#F4F2FF" }}>
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-violet-100/60 px-4 md:px-6 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] text-violet-400 font-medium tracking-wide">Student Resume Builder</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={()=>{setData(SAMPLE);setTplId("gradient-fresh");setAccentOverride(null);}} className="text-xs text-gray-400 hover:text-gray-700 flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-gray-100 transition-colors font-medium">
            <RotateCcw className="w-3.5 h-3.5"/>Sample
          </button>
          <button onClick={()=>setData(EMPTY)} className="text-xs text-gray-500 hover:text-gray-800 flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-gray-100 border border-gray-200 hover:border-gray-300 transition-colors font-medium">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,26rem)_minmax(0,1fr)] min-h-[calc(100vh-49px)]">
        ...
        <aside className="bg-white border-r border-violet-50 lg:h-[calc(100vh-49px)] lg:overflow-y-auto scroll-thin">
          <div className="p-5">
            <h2 className="font-bold text-gray-900 text-base mb-1" style={{ fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Build your resume</h2>
